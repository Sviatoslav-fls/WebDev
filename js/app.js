...
let blogItems = document.querySelectorAll('.works__block');
if (blogItems.length > 0) {
	let blogFilters = document.querySelectorAll('.works__item');
	if (blogFilters.length > 0) {
		for (let index = 0; index < blogFilters.length; index++) {
			...
			blogFilter.addEventListener("click", function (e) {
				...
				const blogFilterActive = document.querySelector('.works__item._active');
		...
		function showBlogItems(filter) {
			for (let index = 0; index < blogItems.length; index++) {
				...
				if (filter === 'all' || !filter) {
					...
					if (blogItem.classList.contains('works__block_' + filter)) {
						blogItem.classList.remove('_hide');
					}
				}
			}
...
window.onscroll = function headerScroll() {
	if (pageYOffset > 10) {
		...
		if (prevScrollpos > currentScrollPos) {
			navbar.classList.remove('_unpinned');
			navbar.classList.add('_pinned');
		} else {
			navbar.classList.remove('_pinned');
			navbar.classList.add('_unpinned');
		}
	}
